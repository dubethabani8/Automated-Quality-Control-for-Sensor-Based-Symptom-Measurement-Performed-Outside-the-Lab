...
%
% with variable z.
% Inputs:  y      - n-vector; original signal
%          lambda - scalar; positive regularization parameter
%
% Outputs: x      - n-vector; primal optimal point
...
function [x,status] = utils_filter_l1tf(y,lambda)
...
MAXITER   = 50;     % IPM parameter: max iteration of IPM
MAXLSITER = 25;     % IPM parameter: max iteration of line search
...
for iters = 0:MAXITER
    ...
    % STOPPING CRITERION
    if (gap <= TOL)
        status = 'L1_trend filtering - solved';
        disp(status);
        x = y-D'*z;
        return;
    end
...
x = y-D'*z;
if (iters >= MAXITER)
    status = 'L1_trend filtering - maxiter exceeded';
    disp(status);
    return;
end
